 ...
 *
 * *建议在分别设置矩阵值时，使用set {x，＃}方法，其中'x'是
 * x，y，z或w和＃可以是0,1,2或3，例如setY1。 你应该使用这些功能的原因是
 *因为它将直接映射到矩阵的该部分，而不管内部矩阵是否为列
 *主修与否。 如果矩阵是或者长度为9或16，它将能够确定它是否可以设置值。
 *如果矩阵的大小为9，但您设置为w2，则不会设置该值，并且设置的方法将不带任何返回
 *
 * error.
 * 4*4矩阵
 */
