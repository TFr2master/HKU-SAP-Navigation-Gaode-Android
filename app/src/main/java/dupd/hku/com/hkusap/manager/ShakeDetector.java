...
public class ShakeDetector implements SensorEventListener {
    ...
    static final int MAX_UPDATE_INTERVAL = 5000;
    /**
     * 上一次检测的时间
     */
    public int times=0;
    long mLastUpdateTime;
    long mLastSuccessUpdateTime;
    /**
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (diffTime < MIN_UPDATE_INTERVAL){
            return;
        }
        mLastUpdateTime = currentTime;
        if(times!=0&&mLastUpdateTime-mLastSuccessUpdateTime>MAX_UPDATE_INTERVAL){
            times=0;
            return;
        }
        ...
        if (delta > shakeThreshold) { // 当加速度的差值大于指定的阈值，认为这是一个摇晃
            times+=1;
            mLastSuccessUpdateTime=mLastUpdateTime;
            if(times>2) {
                times=0;
