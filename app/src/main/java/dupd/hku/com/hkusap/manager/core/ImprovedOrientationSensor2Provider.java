...
public class ImprovedOrientationSensor2Provider extends OrientationProvider {
     ...
     */
    public ImprovedOrientationSensor2Provider(SensorManager sensorManager, Context context) {
        super(sensorManager);
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (event.sensor.getType() == Sensor.TYPE_ROTATION_VECTOR) {
            ...
            //从陀螺样本数据中计算出来之后。
            processGyroscope(event.timestamp, event.values);
            ...
            if (!hasGyroScope) {
                ...
                if (accelerometerTimestamp != 0) {
                    float timeDifference = Math.abs((float) (event.timestamp - this.accelerometerTimestamp) * NS2S);
                    if (timeDifference != 0.0F) {
                        float[] values = this.getGyroscopeValues(timeDifference);
                        processGyroscope(event.timestamp, values);
                    }
                }
    ...
    private void processGyroscope(long eventTimestamp, float[] values) {
        if (timestamp != 0) {
